...
var netProfit = function(eqacct, e, incl, cb) {
  var pp = this
  var args = []
  ...
  if ( this.verbose || true ) console.log(lcommand, _.flatten(args).join(" "))
  ...
  ch.on('exit', function(code,sig) {
    ...
    // entry that moves the net of all Income-Expenses into Equity:Retained-Earnings
      ...
      osa=_.filter(outstr.split("\n"),function(str) { return str != ''})
      if ( this.verbose ) console.log("NETPROFIT\n",osa)
    net = accounting.unformat(osa[osa.length-1])  // profit in last line

    if ( err ) {
      process.stderr.write("\nReported errors and warnings (net profit):\n")
      process.stderr.write(err);
    }
...
var retainedEarnings = function(eqacct, e, incl, cb) {
  netProfit(eqacct,e,incl, function(net) {
    var pp = this
    var args = []
    ...
    ch.on('exit', function(code,sig) {
      ...
      //console.log(outstr.split("\n"))
        var ss = _.flatten(
            [_.map(_.filter(outstr.split("\n"),function(str) { return str!='' }), function(s) {
        return s
          ...
          .replace(/^(.*Closing Entry.*$)/g,"$1\n    ; LEGACY: Hack-for-tag-assertions")
             ...
             ["    ",eqacct,"     ",accounting.formatMoney(net, "$ ", 2, "", ".")].join(""),
             ["    Equity:IGNORE     "].join(""),
            ]).join("\n").replace("^\s*\n","")
      ...
      if ( err ) {
        ...
        if ( this.verbose) console.log('RETAINED EARNINGS JOURNAL ENTRY',ss)
      // return the retained earnings journal entry
      cb(ss)
    })
  })
